...
class UniversitySupportBlueprint(BlueprintBase):
    ...
    def run_with_context(self, messages: List[Dict[str, str]], context_variables: dict) -> dict:
        """Override run_with_context to extract channel_id and update context_variables."""
        ...
        logger.debug(f"run_with_context result: {json.dumps(result, indent=2)}")
        return result
    ...
    def create_agents(self) -> Dict[str, Agent]:
        ...
        triage_instructions = (
            ...
            "(from messages). Use extract_canvas_metadata to get channel ID and learning objectives. If the query is "
            "complex (over 50 words), urgent (contains 'urgent'), or needs human help ('help' or 'complex issue'), "
        ...
        learning_instructions = (
            ...
            "assessment details using SQLite tools. If data is missing, say 'I couldn’t find specific objectives, but here’s "
            "a starting point...'. For general academic queries, delegate back to TriageAgent by calling handoff_to_triage()."
        )
        agents["LearningAgent"] = Agent(
            ...
            instructions=lambda context: (
                ...
                f"Teaching Prompt: {get_teaching_prompt(context.get('channel_id', 'default'))}\n"
                f"Objectives: {', '.join(context.get('objectives', extract_learning_objectives('')))}"
            ),
            functions=[
                handoff_to_triage,
                get_teaching_prompt,
...
# Standalone tool functions (defined outside class)
