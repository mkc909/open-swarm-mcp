# blueprints/university/blueprint_university.py
 ...
 class UniversitySupportBlueprint(BlueprintBase):
     ...
     def finalise_response(self, context_variables: dict) -> str:
         """
     ...
     def triage_to_course_advisor(self, context_variables: dict) -> str:
         """
         Handoff to Course Advisor.
         """
         logger.debug("Handing off to Course Advisor.")
         return json.dumps({"tool_calls": [{"function": {"name": "triage_to_course_advisor", "arguments": json.dumps({})}, "id": "handoff_course_advisor", "type": "function"}] , "context_variables": context_variables})

     def triage_to_university_poet(self, context_variables: dict) -> str:
         """
         Handoff to University Poet.
         """
         logger.debug("Handing off to University Poet.")
         return json.dumps({"tool_calls": [{"function": {"name": "triage_to_university_poet", "arguments": json.dumps({})}, "id": "handoff_poet", "type": "function"}] , "context_variables": context_variables})

     def triage_to_scheduling_assistant(self, context_variables: dict) -> str:
         """
         Handoff to Scheduling Assistant.
         """
         logger.debug("Handing off to Scheduling Assistant.")
         return json.dumps({"tool_calls": [{"function": {"name": "triage_to_scheduling_assistant", "arguments": json.dumps({})}, "id": "handoff_scheduling", "type": "function"}] , "context_variables": context_variables})

     # CourseAdvisor handoff (finalisation)
     def course_advisor_finalise(self, context_variables: dict) -> str:
         """
         Finalise interaction from Course Advisor.
         """
         logger.debug("Course Advisor finalising interaction.")
         return json.dumps({"content": self.finalise_response(context_variables), "context_variables": context_variables})

     # UniversityPoet handoff (finalisation)
     def university_poet_finalise(self, context_variables: dict) -> str:
         """
         ...
         if context_variables.get('response_haiku') == 'true':
             ...
             return json.dumps({"content": f"{self.university_poet_haiku()}", "context_variables": context_variables})
         else:
             logger.info(f"UniversityPoet will respond with a normal response. Context: {context_variables}")
             return json.dumps({"content": self.finalise_response(context_variables) , "context_variables": context_variables})

     def university_poet_haiku(self) -> str:
         return f"""
             A student asks why,
     ...
     def scheduling_assistant_finalise(self, context_variables: dict) -> str:
         """
