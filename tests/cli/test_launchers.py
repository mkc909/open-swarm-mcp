...
def test_swarm_cli_install_creates_executable(monkeypatch, tmp_path, capsys):
    import os
    # Create a dummy blueprint file with a valid BlueprintBase subclass
...
class DummyBlueprint(BlueprintBase):
    ...
    def create_agents(self):
        agent = Agent(name="DummyAgent", instructions="Hello from dummy blueprint")
        self.set_starting_agent(agent)
        return {"DummyAgent": agent}
...
""")
    ...
    assert "Blueprint 'dummy_blueprint' installed as CLI utility" in captured
    # Verify that the wrapper script exists
    wrapper_path = os.path.expanduser("~/.swarm/bin/dummy")
